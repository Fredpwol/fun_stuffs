...
function removeElements(k){
    count = 0
    var cleared_cells = []
    let cells_value = new Map() // This is a hash table that contains all the cell and there values.
    while(count < k){
        ...
        if((value != '') && (!cells_value.has(temp_cell))){
            insert_value(temp_row,temp_col,'') // This sets  the current value to null to check its solvable
            for (const key of cells_value.keys()){
                // The for loop gets all the solvable cells and their values then inputs them in the board excluding the current cell
                cell_key = cells_value.get(key) 
                var cell_col = Math.floor(key/9)
                var cell_row = Math.floor(key%9)

                if(cell_col != 0){
                    cell_col = cell_col - 1
                }
                insert_value(cell_row,cell_col,cell_key)
            }
            solveBoard()
            if(isGameOver()){
                cleared_cells.push([temp_row,temp_col])
                cells_value.set(temp_cell,get_value(temp_row,temp_col))
                for (var i = 0; i < cleared_cells.length; i++) {
                    // Sets all the cleared cells back to null.
                    val = cleared_cells[i]
